/* render5.js — Theme 5 "Verde" hydration engine (organic wellness)
      ...
      <article class="service-card reveal">
        <div class="sc-media">
          ${s.image?.url
            ...
            : `<div class="sc-media-icon">${svgForIcon(s.icon || "default")}</div>`
          }
        </div>
        <div class="service-card-body">
          <div class="sc-kicker">Service ${String(i + 1).padStart(2, "0")}</div>
          <h3>${esc(s.title)}</h3>
  ...
  function renderWhyChooseUs(services, about) {
    ...
    wrap.innerHTML = items.map(it => `
      <div class="why-block reveal">
        <div class="wb-icon" aria-hidden="true"></div>
