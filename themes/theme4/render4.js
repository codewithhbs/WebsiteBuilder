/* render4.js — Theme 4 "Obsidian" hydration engine (dark premium)
...
(function () {
  ...
  function renderServices(services, phone, waLink) {
    ...
    wrap.innerHTML = services.map((s, i) => `
      <article class="service-card reveal">
        <span class="sc-index">/ ${String(i + 1).padStart(2, "0")}</span>
        ${s.image?.url
          ? `<div class="sc-media"><img src="${esc(s.image.url)}" alt="${esc(s.title)}" loading="lazy"></div>`
          : `<div class="sc-icon">${svgForIcon(s.icon || "default")}</div>`
        }
  ...
  function renderAreas(seo, phone) {
    ...
    const chip = a => `<a class="area-chip" href="${esc(telHref)}">${esc(a)}</a>`;
    // duplicated once for a seamless marquee loop (CSS translates -50%)
    wrap.innerHTML = areas.map(chip).join("") + areas.map(chip).join("");
  }
