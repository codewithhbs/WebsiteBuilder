...
(function () {
  ...
  function renderBasic(b) {
    ...
    const accent = b.primaryColor || "#0d6efd";
  ...
  function renderHero(slides, services) {
    ...
    const list = slides?.length ? slides : [{
      ...
      ctaText: "Get Started", ctaLink: "#contact", image: null
    }];

    wrap.innerHTML = list.map((s, i) => {
      const bg = s.image?.url
        ? `background-image:url('${esc(s.image.url)}')`
        : `background: linear-gradient(135deg, #0a0f1e 0%, #1a237e 100%)`;
      return `
  ...
  <div class="hero-text-side">
              ...
              <div class="hero-actions">
                ${s.ctaText ? `<a class="btn-hero-primary" href="${esc(s.ctaLink || "#contact")}">${esc(s.ctaText)} →</a>` : ""}
  ...
  function renderAbout(about) {
    ...
    if (about.image?.url && img) { img.src = about.image.url; img.style.display = ""; if (ph) ph.style.display = "none"; }
  ...
  function renderServices(services) {
    ...
    if (!services?.length) {
      wrap.innerHTML = '<div style="text-align:center;color:var(--text-muted)">No services yet</div>';
      return;
    }
  ...
  function initCounters() {
    ...
    if (!ticker) { setTimeout(animateCounters, 500); return; }
    const obs = new IntersectionObserver(entries => {
  ...
  function hidePreloader() {
    const pl = $("#preloader");
    if (pl) { pl.classList.add("gone"); setTimeout(() => pl.remove(), 500); }
  }
